...
$page_title = "Production Vs Cover";
...
$page_nav["data_sheets"]["sub"]["tara"]["sub"]["production_vs_cover"]["active"] = true;
include(DIRECTORY."/inc/nav.php");

// Get table data
$table = $my_db->Get_Tara_Production_Vs_Cover($_SESSION['diary_milk_username'],$chosen_year);
?>
	<!-- ==========================CONTENT STARTS HERE ========================== -->
	<!-- MAIN PANEL -->
			...
			<div class="col-xs-12 col-sm-7 col-md-7 col-lg-5">
				<h1 class="page-title txt-color-blueDark">
					<i class="fa fa-table fa-fw "></i>
						<?=$lang['DATA_SHEETS']?>
					<span>>
						<?=$lang['TARA']?> > <?=$lang['PRODUCTION_VS_COVER']?>
					</span>
				</h1>
			</div>
		</div>
		...
		<section id="widget-grid" class="">
            ...
            <div class="btn-group dropdown">
                ...
                <ul class="dropdown-menu">
                    <? for ($i=0; $i<=5; $i++):?>
                        <li><a href="production_vs_cover.php?Year=<?= date("Y")-$i; ?>"><?echo date("Y")-$i;?></a></li>
                    <? endfor;?>
                </ul>
            </div>
				...
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					...
					<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-3" data-widget-editbutton="false" >
						...
						<header>
							...
							<h2><?=$lang['MILK_QUALITY_REPORT']?></h2>
						</header>
							...
							<div class="widget-body no-padding">
								...
								<table id="datatable_tabletools" class="table table-striped table-bordered table-hover" width="100%">
									<thead>
										<tr>
                                            <?=$lang['COL_INDEX']?>
                                            <?=$lang['COL_MonthlyDistribution']?>
                                            <?=$lang['COL_COVERS_PERCENTAGE']?>
                                            <?=$lang['COL_COVERS_LITER']?>
                                            <?=$lang['COL_ACTUAL_AMOUNTMILK']?>
                                            <?=$lang['COL_ACTUAL_PERCENTAGE']?>
                                            <?=$lang['COL_DEVIATION_PERCENTAGE']?>
                                            <?=$lang['COL_DEVIATION']?>
                                            <?=$lang['COL_NONE_PERFORMANCE']?>
										</tr>
									</thead>
									<tbody>
                                    <?echo $table ?>
...
//	$('#widget-grid .btn-group').find('.dropdown-toggle').on('click', function (ev){
