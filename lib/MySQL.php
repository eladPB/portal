...
//		$sql = "INSERT INTO milk_measurement (sample_date) VALUES ()";
    ...
    function Get_Herd($FarmId,$StartDate,$FinishDate)
    {
        ...
        foreach ($result as $row):
            $table .= "<tr data-indexid=\"".$row['id']."\">
            <td data-tdid=\"sample_date\">" . $row['sample_date'] . "</td>
			<td data-tdid=\"group_number\">" . $row['group_number'] . "</td>
			<td data-tdid=\"sqft_cowshed\">" . $row['sqft_cowshed'] . "</td>
...
//		$sql = "INSERT INTO herd (sample_date) VALUES ()";
        $herd_keys  = array('group_number','sqft_cowshed','num_milked','average_liters_per_cow_per_day','average_liters_per_milked_cow_per_day','num_cows','dry','calves','dry_matter_weight_dairy_dish');
...
//        exit;
    ...
    function Get_Food($FarmId,$StartDate,$FinishDate)
    {
        ...
        foreach ($result as $row):
            $table .= "<tr data-indexid=\"".$row['id']."\">
            <td data-tdid=\"sample_date\">" . $row['sample_date'] . "</td>
			<td data-tdid=\"dose_code\">" . $row['dose_code'] . "</td>
			<td data-tdid=\"num_servings_transfer\">" . $row['num_servings_transfer'] . "</td>
			<td data-tdid=\"transfer_from_group\">" . $row['transfer_from_group'] . "</td>
			<td data-tdid=\"transfer_to_group\">" . $row['transfer_to_group'] . "</td>
    ...
    {
//		$sql = "INSERT INTO herd (sample_date) VALUES ()";
        $herd_keys  = array('dose_code','num_servings_transfer','transfer_from_group','transfer_to_group','food_milk_kg','food_calf_kg','food_dry_kg','food_total_kg','weight_dairy_food_kg','weight_dairy_female_food_kg','weight_calf_food_kg','weight_dry_food_kg','other_food','price_calf_food','price_dry_food','price_dairy_food','price_female_dairy_food','weight_food_kg');
        if (array_search($key,$herd_keys) === FALSE)
            return false;
        if (!is_numeric($farmid) || !is_numeric($rowid))
            return false;
