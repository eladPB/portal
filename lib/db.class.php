...
class DB {
	...
	function countFarmUsers($Farm) {
		return $this->queryUniqueValue ( "SELECT count(users.Id) FROM users , company WHERE CompanyId = company.Id and company.Name = \"$Farm\"" );
	}
	...
	function latestSampeDate($table) {
		return $this->queryUniqueValue ( "SELECT SampeDate FROM $table WHERE Id = (Select max(Id)  FROM $table )" );
	}
    ...
    // Portal Usage
    function Get_Sstrauss_Manufacturer_Invoices($SstraussUserId)
    {
        // Average Chart
        $sql = "SELECT
                   ProductionDate,
